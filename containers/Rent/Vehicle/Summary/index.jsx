...
const RentSummaryLeft = () => {
    ...
    function validate(values) {
        ...
        else if (!validators.isEmailValid(values.email)) errors.email = "Merci d'ajouter un email valide";
    ...
    function handleSubmit(values) {
        ...
        // TODO : HERE WE CALL THE ROUTE FOR /RENT/VEHICLE/POST ?
        router.push({
    ...
    return (
        ...
        <article style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            {!auth?.id ? (
                <div style={{ display: 'flex', width: '500px', justifyContent: 'space-between', border: '1px solid #DEDEDE', borderRadius: '12px', gap: '2em', padding: '30px' }}>
                    <div style={{ display: 'flex', flexDirection: 'column', minWidth: '60%' }}>
                        <h4>Déjà client?</h4>
                        <span style={{ color: '#8b9197' }}>Connectez-vous et gagnez du temps</span>
                    </div>
            ...
            <form onSubmit={formik.handleSubmit}>
                <div style={{ display: 'flex', width: '500px', flexDirection: 'column', border: '1px solid #DEDEDE', borderRadius: '12px', gap: '2em', padding: '30px' }}>
                    ...
                    <Input
                        ...
                        placeholder={"Ex: jean.dupont@gmail.com"}
                        error={formik.touched.confirmEmail && (formik?.errors?.confirmEmail || !!formik?.status?.errors?.confirmEmail)}
                        name={'confirmEmail'}
                ...
                <div style={{ maxWidth: '50%' }}>
                    <Button disabled={!isObjectEmpty(formik.errors) || isObjectEmpty(formik.values)} type="submit">Valider et réserver</Button>
                </div>
            </form>
        </article>
        </FormikProvider>
...
const RentSummaryRight = () => {
    ...
    return (
        <article style={{ display: 'flex', flexDirection: 'column', width: '100%' }}>
            <div style={{ display: 'flex', width: '470px', flexDirection: 'column', border: '1px solid #DEDEDE', borderRadius: '12px', gap: '1em', padding: '30px' }}>
                <div style={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-between' }}>
                    <h2 style={{ margin: 0 }}>Details</h2>
                    <span style={{ cursor: 'pointer', fontWeight: 'bold' }} onClick={() => router.back()}>Modifier</span>
                </div>
                ...
                <span style={{ maxWidth: '50%' }}>{ rent?.vehicle?.vehicle?.name || 'Description' }</span>
                <Divider />
                <div style={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-between', gap: '1em' }}>
                    <div style={{ display: 'flex', flexDirection: 'column' }}>
                        ...
                        <span style={{ color: '#8B9197' }}>{rent?.vehicle?.startAddress?.displayName}</span>
                    </div>
                    <div style={{ display: 'flex', flexDirection: 'column' }}>
                        <span style={{ fontWeight: 'bold' }}>Adresse de d'arrivée</span>
                        <span style={{ color: '#8B9197' }}>{rent?.vehicle?.endAddress?.displayName}</span>
                    </div>
                </div>
                <div style={{ display: 'flex', flexDirection: 'column' }}>
                    <span style={{ fontWeight: 'bold' }}>Date</span>
                    <span style={{ color: '#8B9197' }}>{formatDate(rent?.vehicle?.startDate)}</span>
                </div>
                <div style={{ display: 'flex', flexDirection: 'column' }}>
                    <span style={{ fontWeight: 'bold' }}>Kilométrage</span>
                    <span style={{ color: '#8B9197' }}>{parseInt(rent?.vehicle?.km) || '-- '}km</span>
                </div>
                <div style={{ display: 'flex', flexDirection: 'column' }}>
                    <span style={{ fontWeight: 'bold' }}>Nombre de déménageurs</span>
                    <span style={{ color: '#8B9197' }}>{parseInt(rent?.vehicle?.nbMovingMen || 0)}</span>
                </div>
                <Divider />
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <h2 style={{ margin: 0 }}>Total :</h2>
                    <span>{rent?.vehicle?.vehicle?.price}{CURRENCY.EUR}</span>
                </div>
            </div>
            <div style={{ display: 'flex', borderRadius: '12px', flexDirection: 'column', padding: '41px 30px', gap: '1em', marginTop: '2em', backgroundColor: '#F1F9F5' }}>
                <h2 style={{ margin: 0 }}>Inclus</h2>
                <span style={{ color: '#8B9197' }}>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Aenean viverra posuere vehicula. Nulla erat tortor, lobortis sed commodo et, commodo vel urna. usce ultricies nibh tortor, in pellentesque est ultrices eget</span>
                <span style={{ color: '#8B9197' }}>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Aenean viverra posuere vehicula. Nulla erat tortor, lobortis sed commodo et, commodo vel urna. usce ultricies nibh tortor.</span>
            </div>
        </article>
    )
}
...
const RentSummary = () => (
        <section style={{ display: 'flex', flexDirection: 'row', padding: '56px 15%', gap: '114px' }}>
            <RentSummaryLeft />
            <RentSummaryRight />
        </section>
    )
