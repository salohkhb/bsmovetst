...
const LiftRentSummaryLeft = () => {
    ...
    function handleSubmit(values) {
        // TODO : HERE WE CALL THE ROUTE FOR /RENT/LIFT/POST ?
        router.push({
    ...
    return (
        <article style={{ display: 'flex', flexDirection: 'column', width: '100%' }}>
            <div style={{ display: 'flex', width: '470px', flexDirection: 'column', border: '1px solid #DEDEDE', borderRadius: '12px', gap: '1em', padding: '30px' }}>
                ...
                <span style={{ maxWidth: '50%' }}>{ rent?.lift?.lift?.name || 'Description' }</span>
                <Divider />
                <div style={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-between', gap: '1em' }}>
                    <div style={{ display: 'flex', flexDirection: 'column' }}>
                        ...
                        <span style={{ color: '#8B9197' }}>{rent?.lift?.startAddress?.displayName}</span>
                    </div>
                </div>
                <div style={{ display: 'flex', flexDirection: 'column' }}>
                    <span style={{ fontWeight: 'bold' }}>Date</span>
                    <span style={{ color: '#8B9197' }}>{formatDate(rent?.lift?.startDate)}</span>
                </div>
                <div style={{ display: 'flex', flexDirection: 'column' }}>
                    <span style={{ fontWeight: 'bold' }}>Nombre de déménageurs</span>
                    <span style={{ color: '#8B9197' }}>{parseInt(rent?.lift?.nbMovingMen || 0)}</span>
                </div>
                <Divider />
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <h2 style={{ margin: 0 }}>Total :</h2>
                    <span>{rent?.lift?.lift?.price}{CURRENCY.EUR}</span>
                </div>
            </div>
